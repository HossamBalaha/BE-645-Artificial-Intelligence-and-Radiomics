...
if (doPlotting):
  ...
  visualStoragePath = r"Data/BT Dataset Visualizations"
  # Create the directory for storing visualizations if it doesn't exist.
  os.makedirs(visualStoragePath, exist_ok=True)

classes = os.listdir(datasetPath) # List all classes in the dataset directory.
history = [] # Create a list to store the features extracted from each image.
...
for cls in tqdm.tqdm(classes, desc="Processing classes."):
  ...
  for file in tqdm.tqdm(files, desc=f"Processing class {cls}."):
    ...
    # cv2.threshold is used to convert the mask to binary.
...
featuresDF.to_csv(
  "Data/BT FOF-GLCM Features.csv",  # Path to save the CSV file.
