...
for i in range(len(volFiles)):
  ...
  if (np.sum(caseSeg) == 0):
    print(f"Skipping {volFiles[i]}: empty segmentation mask.")
    continue
  ...
  # Skip if the ROI is empty (background-only).
  if (np.sum(roi) == 0):
    print(f"Skipping {volFiles[i]}: empty ROI after applying mask.")
    continue
  ...
  if (np.sum(cropped) == 0):
    print(f"Skipping {volFiles[i]}: empty cropped region.")
    continue

  # Calculate first-order features for the current volume slice.
  results, hist2D = FirstOrderFeatures2D(cropped, isNorm=True, ignoreZeros=True)
